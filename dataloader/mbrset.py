...
class MBRSETModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            self.train_ds = MBRSETDataset(self.root, split="train", transform=self.train_transform)
            self.val_ds   = MBRSETDataset(self.root, split="val",   transform=self.val_transform)

        if stage == "test":
            self.test_ds  = MBRSETDataset(self.root, split="val",   transform=self.val_transform)

        if stage == "full":
            # use whole dataset as validation
            self.val_ds   = MBRSETDataset(self.root, split="full",  transform=self.val_transform)
